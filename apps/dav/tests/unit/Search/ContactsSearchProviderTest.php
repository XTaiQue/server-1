...
class ContactsSearchProviderTest extends TestCase {
	...
	public function testGetId(): void {
		$this->assertEquals('dav-contacts', $this->provider->getId());
	}
...
//		$this->assertTrue($result1Data['rounded']);
	...
	public function testGetDeepLinkToContactsApp(): void {
		$this->urlGenerator->expects($this->once())
			...
			->willReturn('link-to-route-contacts.page.index/');
		// TODO: fix All%20contacts here
		$this->urlGenerator->expects($this->once())
			->method('getAbsoluteURL')
			->with('link-to-route-contacts.page.index/All%20contacts/uid123~uri-john.doe')
			->willReturn('absolute-url-link-to-route-contacts.page.index/All%20contacts/uid123~uri-john.doe');

		$actual = self::invokePrivate($this->provider, 'getDeepLinkToContactsApp', ['uri-john.doe', 'uid123']);
		$this->assertEquals('absolute-url-link-to-route-contacts.page.index/All%20contacts/uid123~uri-john.doe', $actual);
	}
